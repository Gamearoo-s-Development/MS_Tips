...
public sealed class RandomTipsPlugin : BaseUnityPlugin
{
    ...
    private void OnCheckoutPaid(float checkoutAmount)
    {
        ...
        var tipChance = Mathf.Clamp01(0.10f + serviceScore * 0.80f);
        ...
        if (Random.value > tipChance)
        {
            ...
            {
                Logger.LogInfo($"No tip this checkout. score={serviceScore:F2}, stock={stockScore:F2}, chance={tipChance:F2}");
            }
            return;
        }
        ...
        if (tipAmount > 0)
        {
            ...
            Logger.LogInfo($"Customer tip received: ${tipAmount:0.00}");
